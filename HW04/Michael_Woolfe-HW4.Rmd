...
```{r}
## Relationship between stolen bases [SB] and runs per game [R]
library(gridExtra)
...
## Report coefficients
fit_1A <- tab %>% lm(SB_per_game ~ R_per_game, data=.)
fit_1A$coefficients
summary(fit_1A)
...
res_1B <- Teams %>% filter(yearID >= 1961) %>%
  mutate(R_per_game = R / G, SB_per_game = SB / G) %>%
  group_by(yearID) %>%
  do(tidy(lm(SB_per_game ~ R_per_game, data = .))) 
      #%>%  filter(term == "HR")

###Data looks approximately normal 
hist(res_1B$estimate)
qqnorm(res_1B$estimate)
qqline(res_1B$estimate)
###The SE is:
sd(res_1B$estimate)
##So the CI is
mean(res_1B$estimate) + c(-1,1)*qnorm(0.95)*sd(res_1B$estimate)

##according to the QQ Plot stealing bases does not result in making runs
```
...
## Based on the CI (-0.08, 0.24) it seems the recruiting is ineffective, since zero is in the confidence interval.
```
...
## The CI (0.09, 0.71) shows a positive indicator of BB to Runs scored in a game, since zero is not in the interval
```
...
## I am not sure how to get a summarized result of the various Coefficients and p-values to determine which covariant is more valuable in this analysis
...
payroll <- my_salaries %>% group_by(teamID, yearID) %>% summarize(sum(salary))
 ...
 payroll %>% ggplot(aes(yearID, log(payroll$`sum(salary)`)), color = factor(payroll$teamID)) +
  geom_point() ## Sadly, could not get the colors to show correctly.
...
summary(fit_2D)
##According to this data, the team wins around 11 more games each time their budget doubles.

##Printing the Low and High Confidence Interval
...
```{r}
##Per Subramanian, not applying a standardization
library(dplyr)
library(tidyr)
library(ggplot2)
library(gridExtra)
...
##Based on this information, the rest of the teams caught up in 1992, given the negative residual in 1992.
...
## This says that Anaheim Angels did the best Base on Balls per game from 2002:2004
