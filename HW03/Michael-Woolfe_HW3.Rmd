...
race2016 <- pollstr_polls(max_pages = 100000,topic = '2016-president-gop-primary')
#?pollstr_polls
#str(race2016)
```
...
```{r}
#Look at the `topic` component of the `questions` component.
  # race2016$questions$topic

#Pull in the race2016 information into a new variable ;; then remove all NA rows
  results <- race2016$questions
  results <- na.omit(results)
#results$state
 # if (race2016$questions$state != "NA" OR race2016$questions$state != "US") (race2016$questions)
#Remove all entries with US, which denotes a national poll
  results <- subset(results, results$state != "US")
  nrow(results)
#results01$state
```
...
```{r}
# Find how many polls were taken
library(dplyr)
group_by(results, id)
```
...
```{r}
#To find the end data of the polls
race2016$polls$end_date
...
#filtering out all choices to have only Rubio and Trump
  results <- subset(results, results$choice == "Rubio" | results$choice == "Trump")
...
```{r}
#Find the relevant fields from race2016#polls and match to results based on the similar column 'id'
results <- left_join(results, race2016$polls, by = "id")
...
```{r}
##Your code here
# head(race2016$survey_houses, 19)

results <- left_join(results, race2016$survey_houses, by = "id")

# Credit : http://rprogramming.net/rename-columns-in-r/
##Now rename the variable/column name from 'name' to 'house'
results <- (rename(results, house = name))
```
